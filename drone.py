...
class Drone:
    ...
    def set_pos_body(self, x: float, y: float, z: float):
        """
        Position offset relative to drone's current heading.
        x: forward(+) / backward(-)
        y: right(+) / left(-)
        z: down(+) / up(-)
        """    
    ...
    def set_velo_body(self, vx: float, vy: float, vz: float):
        """
        Velocity in BODY frame relative to drone heading.
        vx: forward(+) / backward(-)
        vy: right(+) / left(-)
        vz: down(+) / up(-)
    ...
    def takeoff(self, altitude: float, timeout: float = 30):
        """
        Takeoff to specified altitude in meters relative to home.
        Vehicle MUST be in GUIDED mode and armed before calling this.
        timeout: max seconds to wait before giving up (default 30)
        """
        self._conn.mav.command_long_send(
            self._conn.target_system,
            self._conn.target_component,
    ...
    def _wait_for_position(self, timeout=10):
        """Wait until drone velocity is near zero — position reached."""
        time.sleep(0.5)
        start = time.time()
        while time.time() - start < timeout:
            ...
            if speed < 0.1:  # less than 0.1 m/s = essentially stopped
                return
    ...
    def take_pics(self, max_height: float, height_interval: float, angle_interval: int, auto: bool):
        ...
        max_height:      maximum altitude in meters
        height_interval: climb step in meters between pic sets
        angle_interval:  number of rotations per location (evenly spaced 360°)
        auto:            True = fly automatically, False = prompt user at each step

        Pattern: dice #5 — center + fwd-left, fwd-right, back-left, back-right
        At each height, visits all 5 positions, rotates angle_interval times per position.
        """
        SAVE_DIR = "/home/pi/HEF-Models/trainingpics"
        os.makedirs(SAVE_DIR, exist_ok=True)
        count = len(os.listdir(SAVE_DIR))

        OFFSET = 0.61  # 2 feet in meters
        model_w, model_h = 640, 640
        yaw_step = 360.0 / angle_interval

        # dice #5 positions (x=forward, y=right)
        positions = [
            (0.0,     0.0,    "center"),
        ...
        try:
            ...
            while height <= max_height:
                ...
                for (px, py, pos_name) in positions:
                    ...
                    if px != 0 or py != 0:
                        if auto: self.set_pos_body(px, py, 0)   # add if auto
                        self._wait_or_auto(f"At {pos_name}", auto)

                    for r in range(angle_interval):
                        print(f"[takepics] Rotation {r+1}/{angle_interval}")
                        if auto: self.condition_yaw(yaw_step, speed=20, direction=1, relative=1)  # add if auto
